...
(function ($) {
	...
	$.fn.onScreenKeyboard = function (options) {
        ...
        function changeLanguage(){
            ...
            if ( settings.language === 'en' ){
                ...
                // $('#al_pinyin').show();
                $('.wosk-cn').show();
                $('#en_label').removeClass('active-language');
                $('#cn_label').addClass('active-language');
            }else{
                settings.language = 'en';
                // $('#wosk_cn_pinyin').hide();
                // $('#al_pinyin').hide();
                $('.wosk-cn').hide();
                $('#en_label').addClass('active-language');
                $('#cn_label').removeClass('active-language');
            }   
        }
		...
		$keyboard.on('click', 'li', function () {
			...
			if ($key.hasClass('osk-hide')) {
				...
				$input.blur();
				$keyboardTriggers.removeClass('osk-focused');
                //clean the $alternative for pinyin
                $('#wosk_cn_pinyin').empty();
                $('#al_pinyin').empty();
                $alternative = '';
				return false;
			}
			...
			if ($key.hasClass('osk-backspace')) {
                ...
                if ( settings.language == 'en' ) {
                    ...
                    //add by walker delete alternative
                    $alternative = $alternative.substr(0, $alternative.length - 1);
                    console.log( "after backspace", $alternative );
                    $('#wosk_cn_pinyin').empty();
                    $('#wosk_cn_pinyin').append($alternative);
                    //update 
                    if ( $alternative != '' ) {
                        $('#al_pinyin').empty();         
                        $('#al_pinyin').prepend(renderAleterWords(cn_pinyin[$alternative]));
                    }else{
                        $('#al_pinyin').empty();
                    }
                }
            ...
            if($key.hasClass('osk-alter')){
                // return false;
                character='';
            }
            
            //change input language
            if( $key.hasClass('wosk-switch-lang') ){
                character='';
                changeLanguage();                       
            }
            
            if(  $key.hasClass('wosk-cn-open') ){
                return false;
            }
            
            console.log( character );
            if ( settings.language == 'cn' ) {
                if ( $key.hasClass('wosk-cn-element') ) {
                    ...
                    $('#wosk_cn_pinyin').append(character);
                    console.log( 'click', $alternative );
                    $('#al_pinyin').empty();
                    // $('#al_pinyin').append(renderAleterWords(cn_pinyin[$alternative]));
                    $('#al_pinyin').prepend(renderAleterWords(cn_pinyin[$alternative]));
    ...
    /**
    *  Render the alternative words
    *
    *
    */
    function renderAleterWords(alterWords){
        var html = '';
        if (alterWords) {
            var wordsArr = alterWords.split("");
            // console.log( wordsArr );            
            $.each(wordsArr, function(index, value){
                html += '<li class="wosk-cn-element">' + value + '</li>';
            });
            html = '<ul class="osk-cn-alt">' + html + '</ul>';           
        }
        return html;       
    }
	...
	function renderKeyboard(keyboardId) {
		...
		$keyboard = $(
			'<ul id="' + keyboardId + '">' +
                ...
                '<li class="wosk-cn-pinyin wosk-cn" id="wosk_cn_pinyin" style="display:none;"></li>' + 
                '<li class="osk-alter wosk-cn" id="al_pinyin" style="display:none;"></li>'+
                '<li class="wosk-cn wosk-cn-open" style="display:none;">open</li>'+
				'<li class="osk-symbol wosk-new-line">' +
				...
				'<li class="osk-space wosk-cn-element">space</li>' +
                '<li class="wosk-switch-lang osk-last-item">'+
                '<span id="cn_label">中</span>/'+
                '<span id="en_label" class="active-language">EN</span></li>'+
			'</ul>'
		);
